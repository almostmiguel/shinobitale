...
- **Touch**: Affects a target/people you can touch. Touching an unwilling target requires a **Melee Combat (MC)** test against their defensive action. You cannot perform a melee attack and a touch technique simultaneously, nor use two touch techniques at once.
...
- **Touch or ranged powers** require a **MC** (touch) or **CD** (ranged) test if the target resists. For multi-target attacks, one hit test is rolled, but each target defends individually.
